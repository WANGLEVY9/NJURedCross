...
function renderMaterialsBoard(data) {
  ...
  const differences = data.inventory.filter((item) => item.difference !== 0).slice(0, 8).map((item) => {
    const route = item.destinations?.map((destination) => destination.label || (destination.applicant + ' · ' + destination.purpose)).join('；') || item.sourceStatuses?.join('、') || '尚未关联逐笔流转记录';
  ...
  target.innerHTML = '<section class="metric-grid material-metrics">'
    ...
    + materialMetric('登记损耗', data.stats.totalLossQuantity || 0, '归还损耗与报损合计', data.stats.totalLossQuantity ? 'risk' : '')
...
function openStockOperation(operation = '入库', applicationId = '') {
  ...
  const copy = operation === '入库' ? '采购、补充或盘点后新增的物资，登记后会增加可用库存。' : operation === '出库' ? '出库前请核对实物、数量和申请单；提交后会写入库存流水。' : '归还前请核对实物数量与状态；可分别填写实际归还数量和损耗数量。';
  ...
  const destinationField = operation === '出库' ? '<label class="full-field">流转去向 / 使用场景' + (applicationId ? '（可选，默认使用申请用途）' : '') + '<input name="destination" ' + (applicationId ? '' : 'required ') + 'placeholder="例如：急救培训组 / 校园活动现场 / 具体领取人" /></label>' : operation === '入库' ? '<label class="full-field">入库来源 / 批次（可选）<input name="destination" placeholder="例如：采购批次 2026-09-01 / 捐赠单位" /></label>' : '';
  const lossField = operation === '归还' ? '<label>损耗数量<input name="lossQuantity" type="number" min="0" step="1" value="0" required /></label>' : '<input type="hidden" name="lossQuantity" value="0" />';
  showMaterialModal('<p class="overline">INVENTORY TRANSACTION</p><h2>' + title + '</h2><p class="drawer-copy">' + copy + '</p><form id="transaction-form" class="business-form"><input type="hidden" name="operation" value="' + operation + '" /><input type="hidden" name="applicationId" value="' + escapeHtml(applicationId) + '" /><label class="full-field">选择物资<select name="assetCode" required><option value="">请选择或先扫码定位</option>' + options + '</select></label><label>数量<input name="quantity" type="number" min="1" step="1" value="1" required /></label>' + lossField + destinationField + '<label class="full-field">备注<textarea name="note" placeholder="例如：采购批次、归还缺失、盘点说明"></textarea></label>' + photoField + '<div class="interaction-checklist"><b>提交前确认</b><span>✓ 物资和位置已核对</span><span>✓ 数量与实物一致</span>' + (operation === '出库' ? '<span>✓ 借出照片已选择</span>' : '') + (operation === '归还' ? '<span>✓ 已核对归还数量和损耗数量</span>' : '') + '<span>✓ 操作将写入物资流水表</span></div><p class="login-error" id="transaction-error"></p><footer><button class="button secondary" data-close-material-modal type="button">取消</button><button class="button primary" type="submit">预览并确认</button></footer></form>');
}
async function submitTransaction(form) {
  ...
  const lossQuantity = Number(values.lossQuantity || 0);
  if (!Number.isInteger(quantity) || quantity <= 0) { error.textContent = '数量必须是正整数。'; return; }
  if (!Number.isInteger(lossQuantity) || lossQuantity < 0 || lossQuantity > quantity) { error.textContent = '损耗数量必须在 0 到归还数量之间。'; return; }
  ...
  const confirmText = values.operation + '确认\n\n物资：' + item.name + '\n位置：' + item.cabinet + ' · ' + item.level + '\n数量：' + quantity + ' ' + item.unit + (values.operation === '归还' ? '\n损耗：' + lossQuantity + ' ' + item.unit : '') + (values.destination ? '\n去向：' + values.destination : '') + '\n库存变化：' + item.quantity + ' → ' + after + '\n\n确认写入物资流水表吗？';
  ...
  try {
    ...
    payload.set('lossQuantity', String(lossQuantity));
